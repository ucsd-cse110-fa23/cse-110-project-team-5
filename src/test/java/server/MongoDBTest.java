...
public class MongoDBTest {
    ...
    public void testCreateRecipe() {
        mongoDB.createAndUpdateRecipe("abcd", "test name", "dinner", "test details", "testurl");
        try (MongoClient mongoClient = MongoClients.create(uri)) {
            MongoDatabase sampleTrainingDB = mongoClient.getDatabase("Pantry_Pal");
            MongoCollection<Document> usersCollection = sampleTrainingDB.getCollection("Users");
            this.user = usersCollection.find(eq("username", "abcd")).first();
    ...
    public void testUpdateRecipe() {
        mongoDB.createAndUpdateRecipe("abcd", "test update name", "dinner", "test update details", "testurl");
        try (MongoClient mongoClient = MongoClients.create(uri)) {
            MongoDatabase sampleTrainingDB = mongoClient.getDatabase("Pantry_Pal");
            MongoCollection<Document> usersCollection = sampleTrainingDB.getCollection("Users");
            this.user = usersCollection.find(eq("username", "abcd")).first();
        ...
        mongoDB.createAndUpdateRecipe("abcd", "test update name", "new tag", "new details", "testurl");
        try (MongoClient mongoClient = MongoClients.create(uri)) {
            MongoDatabase sampleTrainingDB = mongoClient.getDatabase("Pantry_Pal");
            MongoCollection<Document> usersCollection = sampleTrainingDB.getCollection("Users");
            this.user = usersCollection.find(eq("username", "abcd")).first();
    ...
    public void testReadRecipe() {
        ...
        mongoDB.createAndUpdateRecipe("test read", "test read recipe", "lunch", "test read recipe details", "testurl");
    ...
    public void testReadAllRecipes() {
        ...
        mongoDB.createAndUpdateRecipe("test7", "recipe 1", "lunch", "details 1", "testurl");
    ...
    public void testDeleteRecipe() {
        ...
        mongoDB.createAndUpdateRecipe("test delete", "recipe delete", "lunch", "details delete", "testurl");
